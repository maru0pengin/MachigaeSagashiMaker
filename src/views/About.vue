<template>
  <div class="about">
    ...
    <div id="canvas" class=canvas>main</div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      createGameScene(resources) {
            ...
            difference.obj.on('pointerdown', function () { // クリック時に発動する関数
                if (difference.status === 0) {
                  //console.log(correctCircles[0])
                    ...
                    correctCircles[i].obj.drawCircle(difference.x, difference.y - radius / 2, radius, radius)
        ...
        this.text = new PIXI.Text(`間違い:0/${this.differences.length}`, textStyle); //スコア表示テキスト
        this.text.y = 3;
        this.text.x = 3;
      ...
      */
      ...
      gameLoop() // 毎フレームごとに処理するゲームループ
      {
        //console.log(this.differences)
        // スコアテキストを毎フレームアップデートする
        //let aiueo =  this.differences.filter(word => word.status === 1);
        let aiueo =  this.differences.filter(function(difference){
          //console.log(difference.status)
          return difference.status === 1
        })

        this.text.text = `間違い:${aiueo.length}/${this.differences.length}`;
        ...
        if (aiueo.length === this.differences.length) {
          console.log("test")
...
/* 
        ...
        // テキストオブジェクトの定義
        const text = new PIXI.Text(`${this.displayTimmer}秒で間違えを\n見つけられました！`, textStyle); // 結果画面のテキスト
        ...
        const tweetButton = this.createButton("ツイート", 120, 60, 0x82cddd, () => {
            //ツイートＡＰＩに送信
            //結果ツイート時にURLを貼るため、このゲームのURLをここに記入してURLがツイート画面に反映されるようにエンコードする
            const url = encodeURI("https://machigae-game.web.app/"); // ツイートに載せるURLを指定(文字はエンコードする必要がある)
            window.open(`http://twitter.com/intent/tweet?text=${this.displayTimmer}秒で間違えを\n見つけられました！&url=${url}`); //ハッシュタグをsampleにする
        });
