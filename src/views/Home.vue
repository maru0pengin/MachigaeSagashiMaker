...
export default {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.$notify({ type: 'error', title: 'Error', message: '写真の読み込みに失敗しました。時間をおいて再度お試しください' })
      })
    },
    save() {
      this.date = new Date()
      ...
      console.log(ref)
      ref.put(file).then(() => {
        this.$notify({ type: 'success', title: 'Success', message: 'リファレンスチェックへのご協力ありがとうございました' })
        this.goNext()
      }).catch(err => {
        this.$rollbar.error(err)
        this.$notify({ type: 'error', title: 'Error', message: '名刺のアップロードに失敗しました。時間をおいて再度お試しください' })
      })
    },
    goNext() {
